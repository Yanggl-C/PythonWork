...
class Ui_MainWindow(object):
    ...
    def call_algorithm(self,cmd_type,params = None):
        if cmd_type  not in self.cmb_algorithm:
            QMessageBox.warning(None,'错误','指令错误！')
            return
        self.cmb_algorithm[cmd_type](params)
    ...
    def btn_procressimg_clicked(self):
        ...
        if str_algorithm == "Bgr2Gray":
            gray = self.algorithm.Bgr2Gray(self.img_cp)
            if gray is not None:
                self.show_img(True, gray, self.lab_showImg,self.scrollAreaWidgetContents)
        elif str_algorithm == "Bgr2YUV":
            yuv = self.algorithm.Bgr2YUV(self.img_cp)
            if yuv is not None:
                self.show_img(False, yuv, self.lab_showImg,self.scrollAreaWidgetContents)
        elif str_algorithm == "Bgr2HSV":
            hsv = self.algorithm.Bgr2HSV(self.img_cp)
            if hsv is not None:
                self.show_img(False, hsv, self.lab_showImg,self.scrollAreaWidgetContents)
        elif str_algorithm == 'Bgr2Lab':
            lab = self.algorithm.Bgr2Lab(self.img_cp)
            if lab is not None:
                self.show_img(False, lab, self.lab_showImg,self.scrollAreaWidgetContents)
        elif  str_algorithm == 'MinMaxLoc':
            show_img = self.algorithm.MinMaxLoc(self.img_cp)
            if show_img is not None:
                self.show_img(False, show_img, self.lab_showImg,self.scrollAreaWidgetContents)
        elif str_algorithm == 'Mean':
            show_img = self.algorithm.Mean(self.img_cp)
            if show_img is not None:
                self.show_img(False, show_img, self.lab_showImg,self.scrollAreaWidgetContents)
        elif str_algorithm == 'MeanstdDev':
            show_img = self.algorithm.MeanstdDev(self.img_cp)
            if show_img is not None:
                self.show_img(False, show_img, self.lab_showImg,self.scrollAreaWidgetContents)
            ...
            if self.file_path2 == '' or self.img2 is None:
                ...
                return
            show_img = self.algorithm.Max(self.img_cp,self.img2)
            if show_img is not None:
                self.show_img(False, show_img, self.lab_showImg,self.scrollAreaWidgetContents)
            ...
            if self.file_path2 == '' or self.img2 is None:
                QMessageBox.warning(None,'错误','请加载另一个图像！')
                return
            show_img = self.algorithm.Min(self.img_cp,self.img2)
            if show_img is not None:
                self.show_img(False, show_img, self.lab_showImg,self.scrollAreaWidgetContents)
            ...
            if self.file_path2 == '' or self.img2 is None:
                QMessageBox.warning(None,'错误','请加载另一个图像！')
                return
            show_img = self.algorithm.bitwise_and(self.img_cp,self.img2)
            if show_img is not None:
                self.show_img(False, show_img, self.lab_showImg,self.scrollAreaWidgetContents)
            ...
            if self.file_path2 == '' or self.img2 is None:
                QMessageBox.warning(None,'错误','请加载另一个图像!')
                return
            show_img = self.algorithm.bitwise_or(self.img_cp,self.img2)
            if show_img is not None:
                self.show_img(False, show_img, self.lab_showImg,self.scrollAreaWidgetContents)
            ...
            if self.file_path2 == '' or self.img2 is None:
                QMessageBox.warning(None, '错误', '请加载另一个图像!')
                return
            show_img = self.algorithm.bitwise_xor(self.img_cp, self.img2)
            if show_img is not None:
                self.show_img(False, show_img, self.lab_showImg,self.scrollAreaWidgetContents)
        elif str_algorithm == 'bitwise_not':
            show_img = self.algorithm.bitwise_not(self.img_cp)
            if show_img is not None:
                self.show_img(False, show_img, self.lab_showImg,self.scrollAreaWidgetContents)
        elif str_algorithm == "Threshold":
            ...
            self.openDialog(str_algorithm)
        elif str_algorithm =="Vconcat":
            if self.file_path2 =="" or self.img2 is None:
                QMessageBox.warning(None,'错误','请加载另一个图像！')
                return
            show_img = self.algorithm.Vconcat(self.img_cp,self.img2)
            if show_img is not None:
                self.show_img(False, show_img, self.lab_showImg,self.scrollAreaWidgetContents)
        elif str_algorithm == "Hconcat":
            if self.file_path2 == "" or self.img2 is None:
                QMessageBox.warning(None, '错误', '还添加另一个图像！')
                return
            show_img = self.algorithm.Hconcat(self.img_cp, self.img2)
            if show_img is not None:
                self.show_img(False, show_img, self.lab_showImg,self.scrollAreaWidgetContents)
        elif str_algorithm == "Flip":
             self.openDialog(str_algorithm)
...
class Algorithm(Ui_MainWindow):
    ...
    def LUT(self,rec_img,limit1,limit2,lutnum1,lutnum2,lutnum3):
        ...
        try:
            ...
            QMessageBox.warning(None, "错误", "{}".format(str(e)))
            return
    def Vconcat(self,rec_img1,rec_img2):
        '''
        垂直拼接
        :param rec_img1: 图像1
        :param rec_img2: 图像2
        ...
        try:
            if len(rec_img1.shape) == 3 and len(rec_img2.shape) == 3:
                return cv2.vconcat([rec_img1,cv2.resize(rec_img2,(rec_img1.shape[1],rec_img2.shape[0]))])
            else:
    ...
    def Hconcat(self,rec_img1,rec_img2):
        '''
        水平拼接
        :param rec_img1: 图像1
        :param rec_img2: 图像2
        ...
        try:
            if len(rec_img1.shape) == 3 and len(rec_img2.shape) == 3:
                return cv2.hconcat([rec_img1,cv2.resize(rec_img2,(rec_img2.shape[1],rec_img1.shape[0]))])
            else:
                if len(rec_img1.shape) == 3:
                    rec_img1 = cv2.cvtColor(rec_img1,cv2.COLOR_BGR2GRAY)
                elif len(rec_img2.shape) == 3:
